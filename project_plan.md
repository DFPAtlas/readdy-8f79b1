...
- `/deadlines` — Statutory deadline & notice calendar (protected)
...
- [x] Phase 18: AI Assistant and Document Intelligence — 16 new database tables with RLS (ai_organisation_settings, ai_provider_configs, ai_conversations, ai_messages, ai_runs, ai_run_sources, ai_feedback, document_ingestion_jobs, document_extractions, document_extracted_fields, document_chunks, embedding_jobs, ai_usage_ledger, ai_budget_limits, ai_prompt_templates, ai_draft_links), 6 new enums, 38 indexes, BuildNerve AI edge function (site-ledger-assist) with RAG, chat, conversations, feedback, settings, usage monitoring, prompt injection detection, emergency pattern detection, budget enforcement, output sanitisation, global Assist panel (desktop resizable panel + mobile full-height sheet) with conversation history, source citations, feedback, contextual scope, AI & Automation admin settings page (feature toggles, budget, role-based access, redaction rules, data controls, privacy notice), platform admin AI monitoring page (aggregate adoption, runs, cost, safety blocks, queue health, feedback overview), AI & Automation sidebar nav entry, Assist button in TopBar, 5 seeded prompt templates (job briefing, document Q&A, invoice extraction, certificate extraction, RAMS extraction), OpenAI provider config seeded
- [x] Phase 19: Statutory Deadline & Notice Calendar — 2 new tables with RLS (deadline_rules, statutory_deadlines), statutory_deadline_type + statutory_deadline_status enums, jobs.contract_type column, auto-create trigger on payment_applications/variations/retention_records with per-contract-type built-in defaults, reminder enqueue function writing to notification_outbox, deadline-reminders edge function, /deadlines calendar + list page with filters, home dashboard deadline banner, deadline chip on job detail
- [x] Phase 20: AI Contract Parsing on Job Setup — 2 new tables with RLS (contract_documents, contract_extracted_terms), contract-extraction edge function (upload/extract/confirm, writes confirmed terms to jobs + seeds deadline_rules), contracts service, "Upload contract" wizard step with AI term extraction review + confidence indicators + commercial pre-fill, contract + extracted-terms viewer on job detail documents tab
- [x] Phase 21: RAMS, Method Statements & CDM Compliance — 3 new tables with RLS (rams_documents, toolbox_talks, cdm_duty_holders) + cdm role CHECK constraint, rams-generator edge function (drafts RAMS from job scope + hazard categories with knowledge-base fallback + optional OpenAI tailoring), health-safety service, rams_generation prompt template, Health & safety tab on job detail (RAMS list with draft→reviewed→approved workflow, toolbox talk log, CDM duty holder register), "Generate RAMS with Nerve" modal
- [x] Phase 22: AI Site Photo Analysis (Hazards & Quality) — photo_analyses table with RLS (analysis_type CHECK hazard/quality/defect, JSONB findings with severity, reviewed_by_human + dismissed), photo-analysis edge function (analyze/list/review with deterministic checklist fallback + optional OpenAI vision), evidence service methods, photo-analysis mock + checklist helpers, PhotoAnalysisPanel on evidence detail (analyse with Nerve, severity badges, mark reviewed / dismiss), AI findings badge + "AI findings" quick filter in evidence workspace and job detail evidence tab
- [x] Phase 23: Snagging & Defects Management — snagging_items table with RLS (defect_type CHECK snag/defect, severity CHECK low/medium/high/critical, status CHECK open/in_progress/resolved/closed, JSONB photo_urls), snagging-generator edge function (draft a snag list from trade + scope with deterministic trade knowledge base fallback + optional OpenAI tailoring), snagging service + mock, SnaggingTab on job detail (summary cards, status/type filters, snag→defect→resolved→closed workflow with resolution notes, add-snag modal), "Generate snag list with Nerve" modal, snagging_generation prompt template
